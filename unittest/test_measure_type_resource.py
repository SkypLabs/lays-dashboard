...
from dashboard.models import MeasureType

class MeasureTypeResourceTest(ResourceTestCase):
	def setUp(self):
		super(MeasureTypeResourceTest, self).setUp()

		self.username = 'user'
		self.password = 'pass'
		self.user = User.objects.create_user(self.username, 'user@example.com', self.password)

		self.entry = MeasureType.objects.create(name='Temperature')
		self.list_url = '/api/v1/measure_type/'
		self.detail_url = '/api/v1/measure_type/{0}/'.format(self.entry.pk)
		self.post_data = {
			'name': 'Humidity'
		}
	...
	def test_get_detail_json(self):
		...
		self.assertValidJSONResponse(resp)
		self.assertEqual(self.deserialize(resp)['name'], 'Temperature')
	...
	def test_post_list_json(self):
		self.user.user_permissions.add(Permission.objects.get(codename='add_measuretype'))
		...
		self.assertEqual(MeasureType.objects.count(), 2)
	...
	def test_delete_detail(self):
		self.user.user_permissions.add(Permission.objects.get(codename='delete_measuretype'))
		self.assertEqual(MeasureType.objects.count(), 1)
		self.assertHttpAccepted(self.api_client.delete(self.detail_url, format='json', authentication=self.get_credentials()))
		self.assertEqual(MeasureType.objects.count(), 0)
